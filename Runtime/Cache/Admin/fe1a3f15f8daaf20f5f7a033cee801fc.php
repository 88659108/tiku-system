  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="lighter row">
        <form action="/index.php/Admin/Question/objective" method="get">
            ...
            <div class="lighter row">
              ...
              <div class="col-sm-3">
                <select style="width:80px;" class="form-control" name="field">
                  ...
                  <?php if(($field) == "body"): ?>selected="selected"<?php endif; ?>
                  ...
                  <?php if(($field) == "description"): ?>selected="selected"<?php endif; ?>
                  
                   value="description">解析
                  
                  </option>
                  <option 
                  
                  <?php if(($field) == "id"): ?>selected="selected"<?php endif; ?>
                  ...
                  <option <?php if(($field) == "pointtype1"): ?>selected="selected"<?php endif; ?> value="pointtype1">旧标签</option>
                </select>
              </div>
            </div>
          </div>
          <div class="input-group col-sm-5">
            <input type="text" class="form-control" name="keyword" placeholder="请输入 '试题标题'或'试题解析' 进行模糊查询" value="<?php echo ($_GET['keyword']); ?>">
      ...
      <table class="table table-striped table-hover table-bordered">
        ...
        <tbody>
          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$qt): $mod = ($i % 2 );++$i;?><tr class="js-tr-li-box" dataid="<?php echo ($qt["id"]); ?>">
              ...
              <td><a href="/index.php/Admin/Question/objective_form?id=<?php echo ($qt["id"]); ?>" target="_blank"><?php echo ($qt["id"]); ?></a></td>
              <td><?php echo (htmlspecialchars_decode($qt["body"])); ?>
                <?php if($qt["bd_desc"] != ''): ?><i class="icon-camera"></i><?php endif; ?>
                <?php if($qt["materialid"] > 0): ?><i class="icon-book"></i><?php endif; ?></td>
              <td><span class="label label-success arrowed-in-right"><?php echo ($qt["answer"]); ?></span></td>
              <td><span class="label label-xlg label-grey arrowed arrowed-right"><?php echo (get_question_class_cols($qt["bigtype"])); ?></span>
              <span class="label label-xlg label-light arrowed-in-right arrowed-in"><?php echo (get_question_class_cols($qt["smalltype"])); ?></span><span class="label label-xlg label-light arrowed-in-right arrowed"><abbr title="Phone"><?php echo (get_question_class_cols($qt["pointtype"])); ?></abbr></span><br /><br />
              <?php if($qt["smalltype"] == '0' || $field == 'pointtype1'): ?><span class="label label-xlg label-light arrowed-in-right arrowed-in js-cls-change-text">脏标签：<?php echo ($qt["pointtype1"]); ?></span><?php endif; ?>
              <?php if($field == 'pointtype1' || $qt["smalltype"] == '0'): ?><select style="width:200px;" class="form-control js-cls-change-options" data-qid="<?php echo ($qt["id"]); ?>">
                <?php echo (create_select_options($qt["bigtype"])); ?>
              </select><?php endif; ?>
              </td>
              ...
              <td><div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                  ...
                  <a href="/index.php/Admin/Question/objective_form?id=<?php echo ($qt["id"]); ?>" target="_blank" class="btn btn-xs btn-info"> <i class="icon-edit bigger-120"></i> </a>
                  <a href="/index.php/Admin/Question/objective_ask?id=<?php echo ($qt["id"]); ?>" target="_blank" class="btn btn-xs btn-danger"> <i class="icon-comments-alt bigger-120"></i> </a>
...
$('.js-btn-row-refresh').click(function(){
	...
	var data	= 'diff='+diff+'&status='+status+'&chance=' + chance+'&id=' + dataid;
...
$('.js-all-exe-submit').click(function(){
	...
	var loading = $('.js-all-exe-loading');
		loading.show();
	var data	= 'filed=' + filed + '&value=' + value + '&ids=' + ids.join(',');
...
//旧标签
$('.js-cls-change-options').change(function(){
	...
	var data	= 'id=' + qid + '&bigtype=' + bigid + '&smalltype=' + smallid + '&pointtype=' + pointid;
	ajaxSend('/index.php/Admin/Question/question_update_filed', data, function(json){
		
		if(json.status == 1){
			ops.siblings('.js-cls-change-text').append('--成功变更!');
			return;
		}
		
		ops.siblings('.js-cls-change-text').append('--操作失败!');
	});
