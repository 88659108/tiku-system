 ...
 * paper.navs.js
 * 	
 * 题库模块[试卷菜单栏] v1.0 | @zhongmin | | 2015-12-04
...
;(function($, window, undefined){
	...
	function __htmlstr_objective(data, _number){
		...
		if(!data.uanswer){
			...
			var _materid		= (data.materialid) ? data.materialid : 0;
			var _key			= data.info.qid + '-q-' + _objects.paperid + _materid;
			var _mcdata			= _utils.cache.get(_key);
			if(_mcdata){
				data.uanswer		= _mcdata.uanswer;
				data.answer			= _mcdata.answer;
				data.info.usetime	= _objects.app_paper_time.format(_mcdata.usetime, {msgstr:'用时：', format:'mm分ss秒'});
	...
	function __ajax(data, callback, error){
		...
		_utils.ajax.post(url, {paperid:data.paperid, model:data.model, getinfo:2}, function(json){
	...
	function __save_paper_answer(_callback){
		...
		//请求地址
		var url			= '/Paper/Answer/Index';
		_utils.ajax.post(url, {paperid:_objects.paperid, paperdata:JSON.stringify(_paperdata), getinfo:3}, function(json){
	...
	_lib.extend('navs', {
		...
		create  : function(config){
			...
			_objects.app_paper_time		= _lib.time;
			_objects.app_paper_question	= _lib.question;
